...
fn one_dimensional_ca(w: u32, h: u32, ruleset: &Vec<u8>, chunk_size: u32, pixels: &mut [u8]) {
    ...
    pixels[initial_ind..initial_ind + chunk_size as usize]
        .iter_mut()
        .for_each(|elem| *elem = 1u8);
    ...
    for row_index in 0..h - 1 {
        ...
        (&mut pixels[next_row_start..(next_row_start + (w as usize * chunk_size as usize))])
            .copy_from_slice(&res);
    }
}
...
fn calc_1d_row(row: &[u8], chunk_size: usize, ruleset: &Vec<u8>) -> Vec<u8> {
    ...
    for index in 0..row.len() as isize {
        calculated_row[index as usize] =
            ruleset[((row[wrapped_index(index - 1, row.len() as isize)] << 2)
                + (row[index as usize] << 1)
                + row[wrapped_index(index + 1, row.len() as isize)]) as usize];
    }
...
fn calc_1d_row_chunked(row: &[u8], chunk_size: isize, ruleset: &Vec<u8>) -> Vec<u8> {
    ...
    for start_ind in (0..row.len() as isize).step_by(chunk_size as usize) {
        ...
        let right_neighbour_start_ind =
            wrapped_index((start_ind + chunk_size) as isize, row.len() as isize);
        let left_neighbour_start_ind =
            wrapped_index((start_ind - chunk_size) as isize, row.len() as isize);

        calculated_row[start_ind as usize..end_ind as usize]
            .iter_mut()
            .for_each(|elem| {
                *elem = ruleset[((row[left_neighbour_start_ind] << 2)
                    + (row[start_ind as usize] << 1)
                    + row[right_neighbour_start_ind]) as usize]
            });
    }
    return calculated_row;
}
